...
The architecture of building plots in `ggplot` is similar as in base R graphics. There is some additional call and subsequently more stuff is added to the plot. 

However, in base R, it is sometimes difficult where to add your stuff. For example, think of removing the axis ticks in the scatter plot.

We will systematically explore which elements are used in ggplot in this session. To get an idea how easy it is to create figures in `ggplot`, in the following slides we will have a look at some of the basic plot types you can choose from.
...
```{r load_gapminder_data, echo = TRUE}
gapminder_children <- 
  readr::read_csv(
...
## Difference betwenn `color` and `fill`
When you work with `ggplot`, at least after some time you will be faced by two components of plot or `geom` which are associated with colors: `color` and `fill`.

Generally, `color` refers to the borders of a geometry, such as a line. `fill` refers to an area of a geometry, such as a polygon.

Have this difference in mind when you use `scale_color_brewer` or `scale_fill_brewer` in your plots. Manipulating these colors and their corresponding legends in a complex plot can get really tedious, to be honest. 
...
## Colors and `theme`s
One particular strength of `ggplot2` still lies in its theming capabilities. The package has some built-in theme functions that makes theming a plot fairly easy, e.g.,
- `theme_bw()`
- `theme_dark()`
- `theme_void()`
- etc.
...
## Another remark on manipulations
...
In difference to standard `R` plots `ggplot` are standard objects as any other object in `R` (they are lists). So there is no graphics device involved from which we have record our plot in order to re-use it later. We can just use it like that.
...
As of today, there are now a lot of packages that help combining `ggplot`s fairly easy. For example, the [`cowplot` package](https://cran.r-project.org/web/packages/cowplot/index.html) provides a really flexible framework. I have used `cowplot` to create the map in the previous session.

Yet, fiddeling with this package can be become quite complex. One of my favorite packages at the moment is, therefore, the [`patchwork` package](https://cran.r-project.org/web/packages/patchwork/index.html) because of its easy to use syntax. It's really really easy.
...
I'd recommend, if you're interested, to check out the [`patchwork` repository on Github](https://github.com/thomasp85/patchwork) since everything's really good documented.
