...
Originally, R was created for statistical analysis in 1992. It reaches back to the language `S` from 1976. Here are the caveats we usually find when people talk about R as a programming language:
...
Generally, many ways to interact with the language are similar to using mathematical notation. Please don't get scared of that, yet this is why we deal with vectors, matrices, and so forth. So be prepared.
...
Admittedly, data types and data formats are not an exciting topic. They seem to be a necessary inconvenience rather than something you want to talk about on the Summer School's rooftop party (hopefully) next year. But maybe you want to think about them as containers comprising the exciting information you aim to analyze. Everybody likes some snacks, but nobody the tupperware container with the (vegetarian) gummibears, right?
...
## Numeric data
.small[
*Integers* are values without a decimal value. To be explicit in R in using them you have to place a `L` behind the actual value, just like that:
```{r}
1L
```

In contrast, *doubles* are values with a decimal value
```{r}
1.1
```
...
## Character strings
At first glance, a *character* is  a letter somewhere between a-z. *String* in this context might mean that we have a series of characters. However, also numbers and other symbols can be part of of a *character string*, which then can be, e.g., part of a text. In R, character strings are wrapped in  quotation marks.
...
## Factors
If you're a Stata user, you may be already quite familiar with factors. Factors are data types that assume that their values are not continuous, e.g., as in ordinal or nominal data. They are useful, when inserted into regression models as we will see later in this course.
...
Factors take numeric data or character strings as input as they simply convert them into so-called levels. This concept may be a little bit abstract for the time being. It's just important to have heard about them when you will learn more about them in the data wrangling session.
...
I'd say that logical values are at heart of creating loops, and for this purpose we need way more logical operators to request `TRUE` or `FALSE` values.
...
Similar as the tupperware you can put in your microwave, R data formats can also get quite complex.
...
Using the function `cbind()` or `rbind()` you can either combine vector  column-wise or row-wise, respectively.
...
You cannot store multiple data types, such as strings or numeric values.  Otherwise your data is getting coerced in a common type as we have seen in the previous slide.
...
## Data frames
While matrices are used, e.g.,--\*drumroll\*-- for matrix operations, data frames resembles more what we know of data we usually use. We can build data.frames by hand as here:
...
Lists are perfect to store numerous and potentially diverse information at one place.
...
Generally, there is this use of `[index_number]`-logic in R to access only a subset of information in data, no matter if we had vectors or data frames.

Say, we want to extract the second element of our `character_vector`object, we  could do that as this:
...
(You can do the same indexing with data frames)
...
Just place a `$`-sign between the data object's and the variable's name.

---

## `[]` in data frames
In programming, as you will learn, we often have to rely on character strings as input information, e.g., for iterating over data. Therefore, we can use `[]` to access variables. 
...
But we'll leave it at that for now, so don't worry too much about this topic.

---

## Functional Programming: In R, everything's a function (more or less)
So you might already be familiar with using functions in R (at least we have used them heavily in the preceding slides). Functions are applied as shown here:
...
Later on, we will learn about pipes `%>%` which help to navigate some issues  with too many levels of nesting

---

## Defining your own function is straightforward
First, let's create a simple function which is pasting together some strings and numbers.
...
## Translation of our fabalous joke into `R`
...
## ifelse() shortcut
There's also a shorcut using the function `ifelse()` when the operations applied to the data are not too complex (or at least embedded in another function as in our case):
...
But be aware that `if()` and `ifelse()` were designed for different purposes and the latter is not always a shortcut rather than data type specific. It is handy when you aim to assign the result, e.g, to an object or a variable in a data frame. Ordinary if else statements are more general and should always be your choice when you are not sure.
...
- it depends on specific purpose what defines this element
  - they can be rows, column, list elements etc.
- thus, it is important to think about the iterator of the specific call
...
So if we had data about a whole programmer gang buying milk but who experience varying availability of eggs, we could apply our function to each one of them. Let's try that out!
...
- provides a nice interface to enter your data
...
- **`sapply()`**
- **`tapply()`**
- `mapply()`, `rapply()`, & `vapply()` is left out
  - These are the functions usually not invited to the family dinner

---

## apply()
The apply() command is useful when you want to fire up a short command across either all columns (option `MARGIN = 2`) _or_ rows (option `MARGIN = 2`).
...
While there are packages for building descriptive table already out there (XXX), this becomes handy when you want to create them yourself.

---

## lapply()
lapply() is for more elaborated operations. However, there are no `MARGIN`  options, so let's see what happens when we use it similarly as before:
...
It might be a little bit uncomfortable, but `lapply()` outputs returns each results of an iterated operation as a list element. Thus, the output of applying the function is a list. 
...
lapply(1:nrow(fancy_data), function (i) {
  ifelse(fancy_data$salary_2019[i] > mean(fancy_data$salary_2019), 1, 0)
})
```

---
## sapply()
`sapply()` is similar to `lapply()`. The minor but significant difference is that it returns vectors instead of lists. This comes handy, when you want to add the results of this function as a new column to your existing data.
...
```{r}
tapply(fancy_data$salary_2019,fancy_data$was_milk_available, mean)
```
...
## Let's end this loop
At this point, we stop with adding more looping functions to our inventory. In `R`, there are definitely more functions that at least implicitly use loops and apply-like functions (for a nice overview, please refer to http://uc-r.github.io/apply_family). 

What we also have left out are some newer `tidyverse` functions, e.g., from the `purrr::`-package. If you have any more questions on that please let us know. But for now we will turn to another feature of modern programming in `R` that will be way more supportive for your daily actions in `R` after you have converted to a full-time R-user after this course:
...
.column-left-half[
Pipes are so nice, you may be tempted to use them all the time. Some of you may even spend hours to create one single data preparation workflow in one single pipe. This is awesome. However, it's not very efficient and also misses the whole purpose of applying pipes which are:
- readability
- reproducibility
]
...
.column-left-half[
In some ways, writing code is similar to writing a novel
- it's a craft
- you set the tone how you want to be perceived
  - your future you
  - and others
  
What helps to be understood is styling of your code
]
...
It's really not uncommon to follow a certain style guide that already exists.

A lot of institutions define styles for several different programming languages,
such as Google's style guide for R: https://google.github.io/styleguide/Rguide.html

It really doesn't matter what style you use as long as you are consice (at least in one single R-file)
