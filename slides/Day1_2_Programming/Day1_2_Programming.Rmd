...
You cannot store multiple data types, such as strings or numeric values.  Otherwise, your data is getting coerced in a common type, as seen in the previous slide.
...
## Arrays
Arrays are stacked vectors, as shown in the figure. I haven't seen too many applications with them, to be honest. But there's definitely a use, probably when implementing estimation methods.
...
## Data frames
While matrices are used, e.g.,--\*drumroll\*-- for matrix operations, data frames resemble more what we know of data we usually use. We can build data frames by hand as here:
...
(You can do the same indexing with `data.frame`s)
...
## `[]` in data frames
In programming, as you will learn, we often have to rely on character strings as input information, e.g., for iterating over data. Therefore, we can use also `[]` to access variables. 
...
But we'll leave it now, so don't worry too much about this topic.

---

## Functional Programming: In R, everything's a function (more or less)
So you might already be familiar with using functions in R (at least we have used them heavily in the previous slides). Functions are applied as shown here:
...
- it depends on specific purpose what defines this element
  - they can be rows, columns, list elements, etc.
- thus, it is crucial to think about the iterator of the specific call
...
If we had data about a whole programmer gang buying milk but who experienced varying availability of eggs, we could apply our function to each of them. Let's try that out!
...
## apply()
The `apply()` function is useful when you want to fire up a short command across either all columns (option `MARGIN = 2`) _or_ rows (option `MARGIN = 2`).
...
While there are packages for building descriptive table already out there (e.g., `psych::describe` ), this becomes handy when you want to create them yourself.
...
---
## sapply()
`sapply()` is similar to `lapply()`. The minor but significant difference is that it returns vectors instead of lists. When you want to add the results of this function as a new column to your existing data, this comes handy.
...
## Let's end this loop
At this point, we stop adding more looping functions to our inventory. In `R`, there are definitely more functions that at least implicitly use loops and apply-like functions (for a nice overview, please refer to http://uc-r.github.io/apply_family). 
...
It really doesn't matter what style you use as long as you are concise (at least in one single `R`-file, right?).
...
`R` can also be used for creating text-based adventure games. Look at the tremendous ["Castle of R"](https://github.com/gsimchoni/CastleOfR), which was designed to test your programming skills using base `R`.

Make sure to check out the [background](http://giorasimchoni.com/2017/09/10/2017-09-10-you-re-in-a-room-the-castleofr-package/) of the programming of the package as well.