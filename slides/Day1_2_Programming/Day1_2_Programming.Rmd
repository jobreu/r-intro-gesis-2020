...
fancy_data <-
  data.frame(
    ...
    was_milk_available = 
      sample(c(TRUE, FALSE), 10, replace = TRUE) # and here?
  )
...
## The apply family
The apply family is aimed to make your life easier, saves you a lot of typing 
time and prevents you from writing explicit loops
...
## apply()
The apply() command is useful when you want to fire up a short command across
either all columns (option `MARGIN = 2`) _or_ rows (option `MARGIN = 2`).

```{r}
# means across columns/variables
apply(our_data[,2], 2, mean)

# means across rows/observations
apply(our_data, 1, mean)
```
...
## lapply()
lapply() is for more elaborated operations. However, there are no `MARGIN` 
options, so let's see what happens when we use it similarly as before:
...
It might be a little bit uncomfortable, but lapply() outputs returns each results
of an iterated operation as a list element. Thus, the output of applying the function
is a list. 
...
```{r}
lapply(1:nrow(mtcars), function (i) {
  
  ifelse(mtcars$mpg[i] > mean(mtcars$mpg), 1, 0)
})
```
