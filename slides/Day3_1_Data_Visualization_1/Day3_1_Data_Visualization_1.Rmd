...
This is no statistics course. And while we also figure that all of you are also familiar with the concept of data visualization, we again want you to list why we think it's essential.
...
- scatterplots, line plots, histograms and the like
]
...
The idea is always to initiate the plot first and then add more and more stuff to the plot. In the same logic, e.g., by calling the `axis()` function, you would also delete plot elements.

We could now also adjust some more parameters, such as colors, axis limits, etc. However, this may create a somewhat far-fetched example. Instead, we will learn from other standard plotting functions in `R` and apply more options there.

---

## Histograms
Although we will turn to exploratory data analysis tomorrow, histograms and similar plots are really handy if you want to see the distribution of a variable. For example, in this way, you can show the distribution of the categorized age variable in the GESIS Panel:
...
## Bar plots
The command `barplot()` generates a bar plot from a frequency table. So we need to build it first. Fortunately, that's really easy and something you will need all the time. For example, to create a frequency table for the categorized age variable in the GESIS Panel, you can use:
...
Everything's so far was more or less grayish, so we use the example of bar plots to first create blue bars for our barplot. For this purpose, we use the `rgb()` function. You could also add hex codes as character strings (see below).
...
.pull-right[
If you'd really want to see all plotting options in standard `R`, you could skim through the `graphics::` namespace:
...
`par()` stands for graphical parameters and is called before the actual plotting function. It prepares the graphics device in `R` to know: hey, plot(s) incoming. So a lot can be done within this function, the most often used options are for indeed telling the device that 2, 3, 4, or `x` plots have to be printed in the device.
...
What is more is that other package developers provide new `R` functions that, in the background, make use of `R`'s plotting techniques. These packages rescue you from writing lengthy `R`-scripts just to get a specific layout of a plot.
...
In a way, ready-made functions for beautiful plots requires again to learn new commands, options, and the like. And sometimes, you may have requirements that cannot even be fulfilled with these functions anyway.

These functions help combine information from different variables in a dataset. However,  sometimes you also want to re-create the same old boring graph repeatedly, e.g., for plotting results from similar but different statistical models.
...
In the next session, we will work with plotting techniques using `ggplot2`. We will learn an alternative and more comfortable way of plotting data or results from different groups, e.g., by exploiting so-called facets.  However, the trade-off is that the input data for the plots require more attention and simply more preparation.
...
As said in the beginning, numerous export formats are baked-in into `R`.
...
Alternatively also with the commands `png()`, `pdf()` or `jpeg()`, for example. For this purpose, you first have to wrap the plot call between one of those functions and a `dev.off()`call.
...
In the next session, we will learn more modern techniques using `ggplot2`. Yet, we still believe that it is worthwhile to get comfortable with base `R` plotting since `ggplot2` sometimes can merely too much for simple data exploration.

**In the end, it's also a matter of taste.**
...
Alright, so you may have gotten the feeling that standard `R` graphics are somewhat antiquated. But this need not be true. Have a look at [this excellent and also short blog post](https://www.jumpingrivers.com/blog/styling-base-r-graphics/). The post shows that even with standard `R` you can create great and publication-ready graphics.
