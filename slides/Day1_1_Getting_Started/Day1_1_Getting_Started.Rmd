...
- Feel free to ask questions at any time
  - if it is a question that everybody can/should hear, use "raise hand" function in *Zoom* and then ask via audio/video
  - you can also use the text chat to ask questions
    - you can send text messages only to us, if others should not be able to read them
...
- now also includes spellchecking features
...
Put simply, a function takes an input, does something with it, and produces some sort of output. Functions typically have arguments. In the simplest case, a function only requires an input (a value or object) as a single argument. 
...
# [Exercise](https://jobreu.github.io/tidyverse-workshop-gesis-2019/exercises/A5_DataWrangling1_exercises_question.html) time `r ji("muscle")``r ji("running_man")`
...
They element part of the `R` universe are its packages. They essentially are collections of functions (and sometimes also datasets) and provide some form of documentation for those.
...
```{r installed_pkgs, echo = T, eval = F}
installed.packages()
```
...
- [`styler`](http://styler.r-lib.org/) package (incl. RStudio add-in) that can be used to "automagically" style single files or even all files in a directory according to the tidyverse style guide
...
```{r, rstudio_autocomplete, out.width = "100%"}
...
*RStudio* offers a wide range of [customization options](https://support.rstudio.com/hc/en-us/articles/200549016-Customizing-RStudio). In the following, we will discuss three of them:
...
```{r, rstudio_settings, out.width = "50%"}
...
The suggestions for regarding the general settings in *RStudio*  and many of the recommendations in the following are based on the freely available online book [What They Forgot to Teach You About R](https://rstats.wtf/).
...
```{r, rstudio_history, out.width = "90%"}
...
By default, *R* saves the command history in a `.Rhistory` file in your current working directory upon exiting. You can also manually save and restore the command history using `savehistory()` and `loadhistory()`. You can also open the `.Rhistory` file with any text editor.
...
The `.Rprofile` file contains `R` code that is run when `R` starts. The `.Rprofile` can, e.g., be used to display a personalized welcome message or set a default *CRAN* mirror.
