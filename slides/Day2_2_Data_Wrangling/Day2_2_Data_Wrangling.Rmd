...
A nice thing about `drop_na()` is that you can also provide variable names as arguments to only filter out cases that have missing values for one or more specific variables (check `?dropn_na` for details).
...
```{r dummy, eval = F}
gesis_panel_corona <- gesis_panel_corona %>% 
  mutate(married = ifelse(marstat == 1, 0, 1))
```
