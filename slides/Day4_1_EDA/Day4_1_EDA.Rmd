...
```{r, across_cartoon, out.width = "95%", echo = F}
...
.smaller[
```{r ref.label = "correlation_grouped", echo = F}
...
Of course, there are more options than the ones we will show for plotting univariate distributions. Which to choose depends on the specific kind of data you have, the question you want to answer with the data, and also - to some degree - personal preferences or what is customary in your field.
...
.pull-right[
```{r ref.label = "bar_chart_basic", echo = F}
```
]
...
.pull-right[
```{r ref.label = "bar_chart_color", echo = F}
```
]
...
.pull-left[
While we are quite used to seeing bar charts, flipping its coordinates, so that it becomes a row chart, can make it easier to read (at least for people from countries in which the reading direction is left to right, top to bottom).

.small[
```{r bar_chart_flip, eval = F}
corona_survey %>% 
  filter(!is.na(choice_of_party)) %>% 
  ggplot(aes(x = choice_of_party, 
             fill = choice_of_party)) + 
...
.pull-right[
```{r ref.label = "bar_chart_flip", echo = F}
```
]
...
.pull-right[
```{r ref.label = "bar_chart_order", echo = F}
```
]
...
.pull-right[
```{r ref.label = "bar_chart_perc", echo = F, message = F}
```
]
...
.pull-right[
```{r ref.label = "bar_chart_custom", echo = F, warning = F}
```
]
...
.pull-right[
```{r ref.label = "bar_chart_num", echo = F, message = F}
```
]
...
.pull-right[
```{r ref.label = "bar_chart_num2", echo = F, message = F}
```
]
...
.smaller[
```{r load & wrangle gapminder data display, eval = F}
...
```{r load & wrangle gapminder data, echo = F, message = F}
life_exp_2018 <- read_csv("../../data/life_expectancy_years.csv") %>%
  pivot_longer(-country,
               names_to = "year",
               values_to = "life_exp") %>% 
...
.pull-right[
```{r ref.label = "hist_binwidth", echo = F, message = F}
```
]
...
.pull-right[
```{r ref.label = "qqplot_gesis_panel", echo = F, message = F}
```
]
...
.pull-right[
```{r ref.label = "qqplot_gapminder", echo = F, message = F}
```
]
...
.pull-right[
```{r ref.label = "ppplot_gapminder", echo = F, message = F}
```
]
...
.pull-left[
If we want to compare the distribution of categorical variables across groups, we can use grouped or stacked barplots. If, e.g., we want to compare the party preferences across education categories in the *GESIS Panel Special Survey on the Coronavirus SARS-CoV-2 Outbreak in Germany* dataset, we could do so with a grouped barplot.

.small[
```{r grouped_barplot, eval = F}
corona_survey %>%
  filter(!is.na(choice_of_party)) %>%
...
.pull-right[
```{r ref.label = "grouped_barplot", echo = F}
```
]
...
.pull-left[
If we want to compare the share of female and male voters for each party, we can use a stacked barplot.

.small[
```{r stacked_barplot, eval = F}
corona_survey %>%
  filter(!is.na(choice_of_party)) %>%
  ggplot(aes(x = choice_of_party, 
             fill = sex)) +
...
.pull-right[
```{r ref.label = "stacked_barplot", echo = F}
```
]
...
.pull-left[
One way to compare, e.g., mean values across groups is a barplot with error bars, sometimes also called a dynamite plot.

.small[
```{r barplot_error, eval = F}
corona_survey %>% 
...
.pull-right[
```{r ref.label = "barplot_error", echo = F, message = F}
```
]
...
.pull-right[
```{r ref.label = "boxplot_basic", echo = F, message = F}
```
]
...
```{r, summary_cartoon, out.width = "95%", echo = F}
...
.pull-right[
```{r ref.label = "boxplot_jitter", echo = F, message = F}
```
]
...
.pull-right[
```{r ref.label = "violin_box", echo = F, message = F}
```
]
...
.pull-right[
```{r ref.label = "scatter_plot", echo = F, message = F}
```
]
...
.pull-right[
```{r ref.label = "scatter_margin", echo = F, message = F}
```
]
---
...
Of course, it is also to include more than just two variables in a plot. For example, you can create grouped scatter plots of map the size of the plots in a scatter plot to another variable (e.g., the population of a country in the *Gapminder* examples). If you want to (visually) explore time series data, [*From Data to Viz*](https://www.data-to-viz.com/) has a section on this or you could, e.g. have a look at the [`timetk` package](https://business-science.github.io/timetk/index.html).
...
.pull-right[
```{r ref.label = "vis_dat", echo = F, message = F}
```
]
...
.pull-right[
```{r ref.label = "vis_miss", echo = F, message = F}
```
]
...
.pull-right[
```{r ref.label = "gg_miss_upset", echo = F, message = F}
```
]
...
.pull-right[
```{r ref.label = "gg_miss_var", echo = F, message = F}
```
]
...
.pull-right[
```{r ref.label = "gg_miss_case", echo = F, message = F}
```
]
...
.pull-right[
```{r ref.label = "gg_miss_fct", echo = F, message = F}
```
]
...
For identifying univariate outliers we can specify lower and upper cutoffs using the formula 25th percentile - 1.5 * interquartile range (IQR) for the lower and 75 percentile + 1.5 * IQR for the upper limit. In `R` this can be done as follows:
...
**NB**: The value of 1.5 * IQR is a rule of thumb. You should always check whether this makes sense for your data. In the above example with the *Gapminder* date, e.g., it most likely does not. An alternative approach is using mean + 2 * SD (or 3 * SD), but, again, this is nothing more than a rule of thumb.
...
.pull-right[
```{r ref.label = "inspect_cat", echo = F, message = F}
```
]
