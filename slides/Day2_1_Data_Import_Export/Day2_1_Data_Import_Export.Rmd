...
In order to code along and to be able to do the exercises, you should store the files (at least from the GESIS Panel) in a folder called `data` that is in the same folder as the other materials for this course.
...
- `tidyverse` and its friends help getting order in all the different tools
...
**In the following slides, we'll jump right in importing data. We use a lot of different packages for this purposes and you don't have to remember everything. It's just for making the point of how agnostic `R` actually is regarding the file type. Later on, we will dive in the specifics of importing.**
...
unicorns <- readxl::read_xlsx("../../data/unicorns/observations.xlsx")
```
...
gp_covid <- 
  haven::read_stata("../../data/gesis_panel_covid19/ZA5667_v1-1-0_Stata14.dta")
```
...
A lot of data you get and find or even collect comes in some sort of flat file, such as CSV. In the social sciences, however, we often deal with proprietary file formats, such as SPSS's .sav or Stata's .dta files. 

What we often find in these data are labels. These labels are used to describe variables or variable values and comprise some specific metadata inherent in these proprietary file formats. 
...
Yet, especially when you exchange your data with colleagues who do not use `R` or when you plan to publish your data (what you should always consider), being able to also export data you manipulated in `R` is just great.
...
## Manipulating labels
I used to be a data ingest and preparation guy at the GESIS Data Archive. For this job, I had to use SPSS or Stata for my work, albeit 'privately' I was working with `R` all the time for my dissertation. I would have let Elon Musk name my first-born child, if I could have been able to perform all these tasks in `R`.

Luckily, the generation after me at least could start to use `R` now for labelling or relabelling data. There are plenty packages, one of those is the `sjlabelled` package from Daniel Lüdecke.

.footnote[The name of his child is X Æ A-12, get it?]
...
Yeah, this requires some tedious manual work that have to be done, at least by somebody. But that's just how it is, even in SPSS or Stata. Indeed, we may want to wait until using it in `R` scales a little bit more. Integrating basic labelling of variables in a pipe workflow, however, is already straightforward:
