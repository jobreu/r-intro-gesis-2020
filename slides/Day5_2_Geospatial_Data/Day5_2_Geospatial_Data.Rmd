...
These object types are rather complex and not easy to work with. For some time now, and this is the cutting-edge development in spatial `R`, we can rely on one excellent `sf` package. It implemented the [ISO 19125](https://www.iso.org/standard/40114.html) standard for geospatial data, called "simple features".
]
...
We won't go in too much detail on all possible features or geometries that can be represented in the format of the simple features. What is most notably for working with such geospatial data is that simple features in R are simple data frames with observations in the rows and variables in the columns. One of the main differences to a plain `data.frame` or `tibble` is that they comprise a `geometry` column and some additional metadata, such as the coordinate reference system.
...
To start with geospatial data, this is all you have to know. Probably due to Google Maps and friends, we are so much used to look at maps all day, such that dealing with geospatial data is no big deal anymore.
...
2. We will then simulate some coordinates for our GESIS Panel data. After that, we do some geospatial operations and add information from our geospatial data as additional attributes to the survey data.
...
The `tmap` package is a great resource to create fast and good looking maps. You should definitely look at the [introductionary vignette](https://cran.r-project.org/web/packages/tmap/vignettes/tmap-getstarted.html) of the package here. What's also great about the package is that you can also create excellent interactive maps using the [`leaflet` framework](https://leafletjs.com/).
...
## Static mapping
We can plot the same map as a static map. This is helpful if you prepare a map for a journal publication or something similar. Under the hood `tmap` creates a map with `ggplot2`, so that you can also use `ggplot2`-specific graphics manipulations.
...
But we are allowed to jointly store them with information extracted from geospatial data. Usually, we thus apply a workflow, as shown on the left side.
]

---

## Load GESIS Panel Data
For our introduction of geospatial data, we again use the GESIS Panel COVID-19 dataset.
...
.tinyish[
.pull-left[
We are not allowed to work with 'real' geocoordinates from the GESIS Panel. Solution: Make some up.
...
.pull-right[
For the linking of soil sealing, we use a fancier method to add this information to the GESIS Panel. We draw circular buffers of 500 and 100 Meters around each geocoordinate. Subsequently, we extract the underlying mean values of soil sealing.
]
...
We can think of several statistical models to analyze such data. In survey data, it's always the right choice to use cluster-robust standard errors, and we now could apply them for our municipality data.

The [`estimatr` package](https://cran.r-project.org/web/packages/estimatr/index.html) provides regression models for clustered data. It lets you choose from a large variety of different standard error calculation. It also resembles the one from `Stata`. Let's try it out!
...
`estimatr` provides a nice output by default:
...
## Conclusion
`R` can nowadays function as a full-blown Geographic Information System (GIS). There is a lot more to do, especially on the side of geospatial data. We could model way more complex relationships between individual geocoordinates in space and their relation to other geospatial attributes, e.g., distance relations.

Also, the analysis proposed here is rather basic. In the geospatial analysis, researchers are often interested in the geographic interdependence of their observations. Therefore, a spatial regression model may sometimes be a better choice.
...
We may have talked about creating [shiny apps](https://shiny.rstudio.com/) during the course. It's a great way to provide an interactive interface to data and visualization. Check out [this shiny app](https://shiny.rstudio.com/gallery/map-twitter-sentiment.html), which lets you visualize the geographic distribution of Twitter conversations and sentiments.
