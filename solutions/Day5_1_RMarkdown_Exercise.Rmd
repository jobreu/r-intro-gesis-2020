...
The structure of this final exercise for the course is a bit different. We want you to create and work with `R Markdown` documents (creating `HTML` output) to go through some of the things we covered and did in the previous sessions. So, there are no coding taks in this document.

We have created a `R Markdown` document (which we have also already knitted to create an `HTML` output file) that demonstrates some of the things you can do/create with `R Markdown` and repeats a few of the topics and steps we went through in the sessions before this one.
...
code_download: true -> The document incudes a button allowing you to download the full code

df_print: paged -> When dataframes are printed in the document they are printed in paged tables

**NB**: To knit the document you need to have the packages it uses installed. These are the following ones:
`rmarkdown`, `knitr`, `tidyverse`, `visdat`, `janitor`, `pander`, `patchwork`, `correlation`, `GGally`, `broom`, `sjPlot`.
...
packages("rmarkdown", "knitr", "tidyverse", "visdat", "janitor", "pander", "patchwork", "correlation", "GGally", "broom", "sjPlot", prompt = F)
```
...
The big task we have for you for this exercise is to create a similar `R Markdown` document for the *GESIS Panel Special Survey on the Coronavirus SARS-CoV-2 Outbreak in Germany* data. Using the `explore_gapminder.Rmd` as a template, we want you to do the following in this document:

1. Load and wrangle the data the same way as before to creat the `corona_survey` subset, but with one difference: Also include the variable `hzcy026a`, rename it to `obey_curfew` and code the value 4 for it as `NA`
...
4. Look at the relative frequencies for the variables `sex`, `age_cat`, `education_cat`, and `choice_of_party` using the `tably(function)` from the `janitor package.
...
11. Turn the output of this model into table with the `broom` package and display it with `knitr::kable()`.
