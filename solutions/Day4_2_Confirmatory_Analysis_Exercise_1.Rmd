...
Alright, there's definitely something in the data we should investigate. Sometimes it is considered as bad practice, but we could try to make a median cut for the dependent variable and run a logistic regression first.
...
# Interpretation:
# With increasing age the probability of thinking it's likely to be hospitalized 
# because of COVID-19 increases. But really?
```

Running one model is not enough. It may be that the assumptions for a logistic regression are not fulfilled or a reviewer simply doesn't like these types of regressions. Instead, she proposes to run a binomial regression but with cauchit link.

```{block, box.title = "3", box.body = list(fill = "white"), box.icon = "fa-star"}
Run the same model with the sole difference of using a cauchit link. How would interpret the different regression coefficients?
```
