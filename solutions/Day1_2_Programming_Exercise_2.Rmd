...
In the last exercise, we already have created our first loop using an `if else` statement. Here we want to deepen our understanding of loops and apply some more of them.

```{block, box.title = "1", box.body = list(fill = "white"), box.icon = "fa-star"}
Take the list you have created in the previous exercise, your fancy function to check whether a object is numeric and apply it to each one of the list elements. Use a `for()` loop for this purpose.
```
...
Using the commands in such a way makes the code somewhat difficult to read and understand. You should have learned that pipes provide a straightforward approach to address this issue.
