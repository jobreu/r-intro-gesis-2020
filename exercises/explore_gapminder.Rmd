...
Given that we only have data on DGP per capita from `r min(gap_gdp$year)` to `r max(gap_gdp$year)`, we will focus on data from the year `r max(gap_gdp$year)` in our analysis. The dataset for `r max(gap_gdp$year)` contains data for `r nrow(gap_2018)` countries from `r n_distinct(gap_2018$continent)` continents. Let's have a look at this dataset.
